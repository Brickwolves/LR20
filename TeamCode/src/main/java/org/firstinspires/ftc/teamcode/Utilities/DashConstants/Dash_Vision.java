package org.firstinspires.ftc.teamcode.Utilities.DashConstants;
import com.acmerobotics.dashboard.config.Config;

@Config
public class Dash_Vision {
    /* ------- VISION --------- */
    // Top rectangle starting percentages
    public static double rectTopX1Percent = 1; public static double rectTopX2Percent = 1.2;
    public static double rectTopY1Percent = 0.55; public static double rectTopY2Percent = 0.62;

    // Bottom rectangle starting percentages
    public static double rectBottomX1Percent = 1; public static double rectBottomX2Percent = 1.2;
    public static double rectBottomY1Percent = 0.65; public static double rectBottomY2Percent = 0.69;

    // Generally, orange should be around 90-100
    public static double orangeMax = 110;
    public static double orangeMin = 80;

    public static int ring_count = 4;
}
